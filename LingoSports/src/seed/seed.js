...
const DELAY_MS = Number.parseInt(
  process.env.DELAY_MS || process.env.DELAY || "250",
  10,
);
const NEW_MATCH_DELAY_MIN_MS = Number.parseInt(
  process.env.SEED_NEW_MATCH_DELAY_MIN_MS || "0",
  10,
);
const NEW_MATCH_DELAY_MAX_MS = Number.parseInt(
  process.env.SEED_NEW_MATCH_DELAY_MAX_MS || "0",
  10,
);
...
const SEED_SOURCE_LOCALE = String(
  process.env.SEED_SOURCE_LOCALE || "en",
).trim().toLowerCase();
const SEED_TRANSLATION_QUALITY = String(
  process.env.SEED_TRANSLATION_QUALITY || "fast",
).trim().toLowerCase();
const SEED_PRECOMPUTE_TRANSLATIONS =
  process.env.SEED_PRECOMPUTE_TRANSLATIONS === "1" ||
  process.env.SEED_PRECOMPUTE_TRANSLATIONS === "true";
const SEED_INCLUDE_GLOBAL_FAN_VIEW =
  process.env.SEED_INCLUDE_GLOBAL_FAN_VIEW === "1" ||
  process.env.SEED_INCLUDE_GLOBAL_FAN_VIEW === "true";
const SEED_RESET_BEFORE_RUN =
  process.env.SEED_RESET_BEFORE_RUN !== "0" &&
  process.env.SEED_RESET_BEFORE_RUN !== "false";
const API_URL = process.env.API_URL;
if (!API_URL) {
  throw new Error("API_URL is required to seed via REST endpoints.");
}
const SEED_USER_AGENT = "lingosports-seed/1.0";
const JSON_HEADERS = {
...
async function resetSeedData() {
  const response = await fetch(`${API_URL}/seed/reset`, {
    method: "POST",
    headers: JSON_HEADERS,
  });
  if (!response.ok) {
    let message = `Failed to reset seed data: ${response.status}`;
    try {
      const payload = await response.json();
  ...
  const matchesResetToZero = Number(payload?.data?.matchesResetToZero || 0);
  console.log(
    `🧹 Reset done. Matches reset=${matchesResetToZero}, cleared commentary=${before.commentary ?? 0}, translations=${before.commentaryTranslations ?? 0}, lingoEvents=${before.lingoEvents ?? 0}`,
  );
}
...
async function insertCommentary(matchId, entry) {
  const payload = {
    ...
    sourceLocale: SEED_SOURCE_LOCALE,
    quality: SEED_TRANSLATION_QUALITY,
    precompute: SEED_PRECOMPUTE_TRANSLATIONS,
    includeGlobalFanView: SEED_INCLUDE_GLOBAL_FAN_VIEW,
  };
...
function extractBasketPoints(entry) {
  const eventType = String(entry.eventType || "").toLowerCase();
  if (eventType === "three") {
    return 3;
  }
...
function scoreDeltaFromEntry(entry, match) {
  ...
  const basketPoints = extractBasketPoints(entry);
  if (basketPoints !== null) {
    if (entry.team === match.homeTeam) {
      return { home: basketPoints, away: 0 };
    }
    if (entry.team === match.awayTeam) {
      return { home: 0, away: basketPoints };
    }
  }
...
function cricketScoreDelta(entry, match) {
  ...
  if (battingTeam === match.homeTeam) {
    return { home: delta.home || delta.away, away: 0 };
  }
  return { home: 0, away: delta.away || delta.home };
}
...
function randomMatchDelay() {
  const minDelay = Math.max(0, NEW_MATCH_DELAY_MIN_MS);
  const maxDelay = Math.max(minDelay, NEW_MATCH_DELAY_MAX_MS);
  const range = maxDelay - minDelay;
  return minDelay + Math.floor(Math.random() * (range + 1));
}
...
async function seed() {
  ...
  for (let i = 0; i < randomizedFeed.length; i += 1) {
    ...
    if (delta && (delta.home !== 0 || delta.away !== 0)) {
      ...
      target.match.homeScore = target.score.home;
      target.match.awayScore = target.score.away;
